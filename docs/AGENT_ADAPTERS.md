...
Support targets Claude Code 2.1.260 or later with the flags checked by `doctor`. Each invocation uses print mode, stdin prompts, a JSON schema, and verbose JSON event streaming. A turn is limited to the configured `agent_turns` (32 by default) and the configured `agent_timeout`. Measurements of that ceiling against this repository's own ticket graph are recorded in [OBSERVED_LIMITS.md](OBSERVED_LIMITS.md). Claude documents these output and limit options in its [programmatic usage guide](https://code.claude.com/docs/en/headless) and [CLI reference](https://code.claude.com/docs/en/cli-reference).
