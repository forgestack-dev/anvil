...
An `adaptive` object requires `profiles`, `defaults` (worker ID or agent name to profile), and `review_profile`. The review profile's agent must match the top-level reviewer. Each profile defines `agent`, `model`, `effort`, and `rank`. A ticket may specify `profile` and `risk` (`low`, `medium`, or `high`). Explicit profile overrides win; worker affinity still applies. No compatible worker is a preflight error. All settings are passed to a fresh invocation, with existing tool and permission boundaries retained.
