...
Learning uses comparable task cohorts, disjoint input-based training/held-out splits, matching profile catalogs, retry limits, reviewer selection, verification commands, execution timeouts and per-agent CLI versions (worker and reviewer), observed acceptance, total cost per accepted ticket, and latency. Repeated identical inputs cannot inflate independent sample counts. Unknown cost, mismatched reported model, injected test runners, and unclassified failures do not justify policy promotion. When the CLI does not report a model, comparison uses the explicitly requested profile and CLI version; actual model identity remains unconfirmed. Training validates no route unless the contributing evidence pins exactly one reviewer CLI version. A successful run demonstrates observed sufficiency, not that the chosen model was optimal. Historical comparisons retain selection bias.
