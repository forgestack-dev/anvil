...
Status: serial execution is implemented, with Codex acceptance established through a live two-ticket exercise. A run can now select Codex or Claude Code for implementation and independent review of the exact integration revision, followed by required checks, a managed local branch, and a SQLite evidence ledger. `anvil status` reads saved runs; it does not resume them. See [ROADMAP.md](ROADMAP.md) for milestone progress, [VALIDATION.md](VALIDATION.md) for evidence, and [README.md](../README.md) for supported commands and configuration.
