...
Status: serial execution and coordinated mixed Codex/Claude/Muse worker pools are implemented. Explicit and deterministic automatic AI Hero text-skill selection, capability preflight, native recovery, and committed Markdown spec preparation are implemented. Codex serial acceptance was established through a live two-ticket exercise; mixed pools, ticket skill execution, automatic selection, and spec preparation have deterministic tests, not a recorded live model acceptance run. One supervisor owns dispatch, resource reservations, dependency handoffs, and review/checks of each exact integration revision on the latest accepted base. See [ROADMAP.md](ROADMAP.md), [VALIDATION.md](VALIDATION.md), and [README.md](../README.md) for supported capabilities and evidence.

The remainder describes the full target design. Legacy execution uses one attempt per ticket on macOS/Linux; adaptive runs allow one bounded review/check escalation. A blocker or terminal failure stops the whole run and cancels active peers. Pause commands, general retries, multi-ticket atomic staging groups, model-assisted skill selection, and binary skill resources remain planned. Exclusive tickets and declared resource labels serialize known overlapping work. Verification commands are trusted configuration executed directly on the host. See [implemented coordination](PARALLEL_EXECUTION.md) for the pool contract and its limits.
