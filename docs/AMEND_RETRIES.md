...
Status: proposed specification; not implemented. Based on `main` at `373e7c1`.
The mechanism in section 3 was prototyped against a real repository at that
revision and its result is reproduced here; nothing else in this document runs.
The measurements it argues from are in [OBSERVED_LIMITS.md](OBSERVED_LIMITS.md),
and the acceptance flow it extends is [ACCEPTANCE.md](ACCEPTANCE.md).

## 1. Outcome

A rejected candidate is discarded whole. When review asks for a change the
candidate is two lines from making, the run pays a full implementation turn to
produce those two lines, starting from an empty worktree and rediscovering
everything the rejected attempt already got right.
...
No change to `commit_candidate`, `prepare_integration`, or the phase machine is
required. The new code is the worktree preparation and the choice of when to use
it.

## 4. The constraint that decides correctness

`retry` rebases the replacement attempt onto the coordinator's *current*
accepted base (`parallel.py:320`), and that base advances whenever a peer
integrates (`parallel.py:446`). A rejected candidate's tree is
...
Today a retry escalates to the next-ranked profile, because the evidence that
the attempt failed is read as evidence that the profile was insufficient. An
...
An amend should keep the profile that produced the candidate. That is a change
to `adaptive.next_profile`'s role at this call site, not to the policy: the
escalation allowance is untouched and remains available if the amend itself is
rejected, which is the point at which capability is genuinely in question.
...
are what an independent review asked to change; and the ticket's criteria are
unchanged and still all apply. The last is load-bearing. A worker told only to
address findings will not re-check the criteria the findings do not mention, and
the amended candidate is reviewed against all of them.
...
paragraph. Today's retry appends them as free text.
...
- A rejected attempt whose base advanced in the meantime falls back to a fresh
  ...
  test that matters; it should be written first and watched fail against a
  version that always amends.
...
- The amended attempt's `routing_decision` names the candidate it amended, and
  that revision resolves.
- Existing retry and escalation tests pass unchanged where the base did advance.
